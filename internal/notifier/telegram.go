...
func (t *TelegramNotifier) Send(integrationKey, to, message string) error {
	...
	// Дополнительно парсим JSON, чтобы убедиться, что Telegram реально вернул success
	...
	t.logger.Info(fmt.Sprintf("Telegram message sent successfully to %s via %s", to, integrationKey))
	return nil
}
